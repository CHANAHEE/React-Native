...
export default function Login(props:LoginProps):JSX.Element{ //props객체{navigation, route}
    ...
    // InputComponent 의 글씨가 변경될 때 마다 발동하는 함수
    const changeText = (text: string)=>{email = text}
    

    // 로그인 버튼 클릭시 실행될 함수
    const login = async ()=>{
        // 원래는 서버에 전송하여 DB 에 저장해야함

        // 디바이스의 저장소에 이메일 정보를 저장하여 다시 앱을 실행할 때 로그인을 재차 물어보지 않도록 AsyncStorage 에 저장
        // async - await 문법을 통해 비동기 작업 완료 후 화면 전환되도록.. 
    ...
    return (
        <View style={style.root}>
            ...
            <View style={style.content}>
                {/* 1.1 로고 */}
                <Text style={style.logo}>MOVIE CHA</Text>
